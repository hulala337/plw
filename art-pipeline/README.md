...
资产文件有统一的 GitHub 同步机制：

- 详细规范：根目录 ART_SYNC_GUIDE.md
- 离线接力：根目录 ART_OFFLINE_HANDOFF_GUIDE.md
- 资产根目录：art-assets/
...
如果图片是在 ChatGPT、其他 AI 平台或其他电脑生成后下载到本机，不要手工猜目录和版本号：

python art-pipeline\\asset_intake.py "C:\\Users\\你的用户名\\Downloads\\生成图片.png"

工具默认读取 ART_PRODUCTION_STATE.json 的 current_asset，校验该 ID 是否存在于 Manifest，并自动选择候选目录中的下一个版本号。

也可以明确指定资产并先预览：

python art-pipeline\\asset_intake.py "D:\\incoming\\pelican.png" --asset-id B04 --dry-run

规则：自动复制而不删除源文件；默认禁止覆盖；候选始终保持 GENERATED / HUMAN REVIEW REQUIRED，不会自动 APPROVED、FROZEN 或推进 STATE。

## Return Package 自动导出 / 校验

如果某个生产环境不能连接 GitHub，但有一份项目快照，可以自动整理当前资产的回传包：
...
只有输出 RETURN PACKAGE: PASS 才交给 GitHub 环境。工具会收集当前资产 candidates、approved、QA、review，以及 STATE、Manifest、ART_HANDOFF.md、ART_SYNC_GUIDE.md，并生成 SHA-256 checksums.json。

Return Package 是运输容器，不代表 APPROVED；不会自动修改 STATE。
