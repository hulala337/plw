...
async function renderWeather(){
  ...
  try{
    ...
    },()=>{if(requestId!==state.weatherRequestId||!weatherEnabled)return;if(cached?.data)show(cached.data,'cache');else if($('weather'))$('weather').textContent='天气暂不可用';},{enableHighAccuracy:false,maximumAge:cacheMaxAge,timeout:5000});
  }catch(e){if(requestId!==state.weatherRequestId||!weatherEnabled)return;if(cached?.data)show(cached.data,'cache');else if($('weather'))$('weather').textContent='天气暂不可用';}
}
